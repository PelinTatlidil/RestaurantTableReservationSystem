...
describe('logout and protected access', () => {
  ...
  test('registration can submit even when email availability check is still pending', async () => {
    ...
    await act(async () => {
      ...
      changeInputValue(inputs[0], 'New Customer');
      changeInputValue(inputs[1], 'new@example.com');
      changeInputValue(inputs[2], '0400123456');
      changeInputValue(inputs[3], 'secret123');
      changeInputValue(inputs[4], 'secret123');
    });

    await waitFor(() => {
      expect(container.textContent).toContain('Checking email...');
...
describe('admin dashboard access and navigation', () => {
  ...
  test('admin can access dashboard and see management links', async () => {
    ...
    expect(container.querySelector('a[href="/tables"]')).toBeTruthy();
...
describe('admin reservation management', () => {
  ...
  test('admin can update and permanently delete an existing reservation after confirmation', async () => {
    localStorage.setItem(
      'user',
      JSON.stringify({
        id: 'admin-id',
    ...
    expect(container.textContent).toContain('Permanently delete reservation for Updated Customer?');
    ...
    await waitFor(() => {
      ...
      expect(container.textContent).not.toContain('Updated Customer');
